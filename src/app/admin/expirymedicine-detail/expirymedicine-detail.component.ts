...
export class ExpirymedicineDetailComponent implements OnInit {
  ...
  checkExpiringSoonAlert(): void {
    ...
    this.service.getExpiringSoonMedicineList(obj).subscribe({
      next: (r1: any) => {
        const response = r1 as any;

        if (response && response.Message === ConstantData.SuccessMessage) {
          const list = Array.isArray(response.ExpiryMedicineList)
            ? response.ExpiryMedicineList
            : [];

          if (list.length > 0) {
            this.toastr.warning(
              `Alert: ${list.length} medicine(s) are expiring within the next 1 month!`,
      ...
      error: (err: any) => {
        /*
         * Don't show a second error popup here.
         * This alert is supplementary.
         */

        console.error('Expiring Soon API Error:', err);
      },
    });
  }
