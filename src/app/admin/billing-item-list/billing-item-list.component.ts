...
export class BillingItemListComponent {
  ...
  this.filterModel = {
   ...
   getPrint(data: any) {
     this.service.PrintOpticlalBill(data.BillingItemId);
   }
   ...
   openViewModal(item: any) {
     ...
     this.billItemSellListPayment(item);
   }
   BillingSellListPayments: any = {};
   ...
   billItemSellListPayment(obj: any) {
     ...
     this.dataLoading = true;
     this.service.billItemSellListPayment(request).subscribe(
       (r1) => {
         let response = r1 as any;
         if (response.Message == ConstantData.SuccessMessage) {
           this.BillingSellListPayments = response.BillingSellListPayments;
           this.dataLoading = false;
         } else {
   ...
   DeliveryStatusUpdate(obj: any) {
     ...
     this.Deliverystatus.DeliveryDate = this.loadData.loadDateYMD(
       this.Deliverystatus.DeliveryDate
     );
 
     var request: RequestModel = {
       request: this.localService
         .encrypt(JSON.stringify(this.Deliverystatus))
         .toString(),
     };
     this.dataLoading = true;
