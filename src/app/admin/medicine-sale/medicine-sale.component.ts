...
export class MedicineSaleComponent implements OnInit {
  ...
  getSaleDetail(PaymentCollectionId: number) {
    ...
    var obj: RequestModel = {
      request: this.localService
        .encrypt(JSON.stringify({ PaymentCollectionId: PaymentCollectionId }))
        .toString(),
    };
    this.service.getSaleDetail(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
  ...
  validiateMenu() {
    var obj: RequestModel = {
      request: this.localService
        .encrypt(
          JSON.stringify({
            Url: '/admin/patient-medicine-sale',
  ...
  keyPress(event: any) {
    ...
    if (keycode == '43') {
      ...
      if (
        this.PaymentCollection.SearchPatient &&
        this.PaymentMedicineList.length > 0
      ) {
        this.saveMedicalPayment();
      } else {
        if (this.Elements.InputPatientAutoComplete == null) {
          var inputDivs: any = document.getElementsByTagName('ng-autocomplete');
          this.Elements.InputPatientAutoComplete =
            inputDivs[
              'PatientAutoComplete'
  ...
  getMedicineStockList() {
    ...
    this.dataLoading = true;
    this.service.getMedicineStockListForSell(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
...
changeQuantity(PaymentMedicineModel: any, IsDiscountAmountChange?: boolean) {
  ...
  PaymentMedicineModel.Quantity = Number(PaymentMedicineModel.Quantity) || 0;
  PaymentMedicineModel.MRP = Number(PaymentMedicineModel.MRP) || 0;
  PaymentMedicineModel.PurchaseUnitValue = Number(PaymentMedicineModel.PurchaseUnitValue) || 1;
  PaymentMedicineModel.UnitValue = Number(PaymentMedicineModel.UnitValue) || 1;
  ...
  PaymentMedicineModel.BasicAmount = this.loadData.round(
    PaymentMedicineModel.Quantity * PaymentMedicineModel.Amount,
    2
  );

  PaymentMedicineModel.DiscountPercentages = Number(PaymentMedicineModel.DiscountPercentages) || 0;
  PaymentMedicineModel.DiscountAmount = Number(PaymentMedicineModel.DiscountAmount) || 0;

  if (IsDiscountAmountChange) {
    PaymentMedicineModel.DiscountPercentages = PaymentMedicineModel.BasicAmount > 0 
      ? this.loadData.round((PaymentMedicineModel.DiscountAmount * 100) / PaymentMedicineModel.BasicAmount, 2) 
      : 0;
  } else {
    PaymentMedicineModel.DiscountAmount = this.loadData.round(
      (PaymentMedicineModel.BasicAmount * PaymentMedicineModel.DiscountPercentages) / 100,
      2
    );
  }

  PaymentMedicineModel.TotalAmount = this.loadData.round(
    PaymentMedicineModel.Quantity * PaymentMedicineModel.Amount -
      (PaymentMedicineModel.DiscountAmount > 0 ? PaymentMedicineModel.DiscountAmount : 0),
    2
  );
  ...
  calculateGST(PaymentMedicine: any) {
    ...
    if (PaymentMedicine.GSTId > 0) {
      ...
      PaymentMedicine.SGSTAmount = this.loadData.round(
        PaymentMedicine.TotalGSTAmount / 2,
        2
      );
      PaymentMedicine.IGSTAmount = 0;
    }
    PaymentMedicine.TaxableAmount = this.loadData.round(
  ...
  changeDiscountAmount(PaymentMedicineModel: any) {
    PaymentMedicineModel.DiscountPercentages = this.loadData.round(
      (PaymentMedicineModel.DiscountAmount * 100) /
        PaymentMedicineModel.BasicAmount,
      2
    );
  }
  ...
  calulateTotal(isPaidAmount?: boolean) {
    ...
    if (this.PaymentCollection.BillingOf != 2) {
      this.PaymentMedicineList.forEach((p1) => {
        this.PaymentCollection.TotalAmount += p1.TaxableAmount;
        this.PaymentCollection.DiscountAmount +=
          p1.DiscountAmount > 0 ? p1.DiscountAmount : 0;
        this.PaymentCollection.CGSTAmount +=
          p1.CGSTAmount > 0 ? p1.CGSTAmount : 0;
        this.PaymentCollection.SGSTAmount +=
          p1.SGSTAmount > 0 ? p1.SGSTAmount : 0;
        this.PaymentCollection.IGSTAmount +=
          p1.IGSTAmount > 0 ? p1.IGSTAmount : 0;
        this.PaymentCollection.PayableAmount += p1.TotalAmount;
      });
      //if (this.PaymentCollection.PaidAmount != null && this.PaymentCollection.PaidAmount != undefined)
      this.PaymentCollection.CGSTAmount = this.loadData.round(
        this.PaymentCollection.CGSTAmount,
        2
      );
      this.PaymentCollection.SGSTAmount = this.loadData.round(
        this.PaymentCollection.SGSTAmount,
        2
      );
      this.PaymentCollection.IGSTAmount = this.loadData.round(
        this.PaymentCollection.IGSTAmount,
        2
      );
      this.PaymentCollection.NetPayableAmount = this.loadData.round(
        this.PaymentCollection.PayableAmount
      );

      if (!isPaidAmount) {
        if (this.PaymentCollection.DiscOnBill > 0) {
          this.PaymentCollection.PayableAmount =
            this.PaymentCollection.NetPayableAmount -
            this.PaymentCollection.DiscOnBill;
      ...
      this.PaymentCollection.PaymentMode = 1;
      this.PaymentCollection.DueAmount =
        this.loadData.round(
          this.PaymentCollection.NetPayableAmount -
            this.PaymentCollection.PayableAmount -
            this.PaymentCollection.DiscOnBill,
    ...
    // }
      ...
      if (!isPaidAmount) {
        ...
        if (this.PaymentCollection.DiscOnBill > 0) {
          this.PaymentCollection.AfterDueAmount =
            this.PaymentCollection.NetPayableAmount -
            this.PaymentCollection.DiscOnBill;
        } else {
          this.PaymentCollection.AfterDueAmount =
        ...
        this.PaymentCollection.PaymentMode = 4;
        this.PaymentCollection.DueAmount =
          this.loadData.round(this.PaymentCollection.AfterDueAmount, 2) ?? 0; // Set DueAmount initially
      }
      ...
      // Recalculate when PaidAmount is updated
        ...
        if (this.PaymentCollection.PaidAmount > 0) {
          ...
          this.PaymentCollection.DueAmount =
            this.loadData.round(
              this.PaymentCollection.NetPayableAmount -
                this.PaymentCollection.PaidAmount -
                this.PaymentCollection.DiscOnBill,
              2
            ) ?? 0;
        } else {
          ...
          this.PaymentCollection.DueAmount =
            this.loadData.round(this.PaymentCollection.AfterDueAmount, 2) ?? 0;
        }
      }
    }
    ...
    this.PaymentCollection.TotalAmount =
      this.PaymentCollection.NetPayableAmount;
  }
  ...
  getSearchGeneralPatientList(GeneralPatientId: number) {
    ...
    this.service.getSearchGeneralPatientList(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
  ...
  getSearchIPDPatientList() {
    ...
    this.service.getSearchPatientList(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
  ...
  saveMedicalPayment() {
    ...
    this.service.saveMedicalPayment(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
          this.toastr.success(
  ...
  filterMedicineList(value: any) {
    if (value) {
      ...
      this.MedicineDetailList = this.MedicineList.filter((med) =>
        med.MedicineName.toLowerCase().includes(filterValue)
      ).slice(0, 50);
    } else {
