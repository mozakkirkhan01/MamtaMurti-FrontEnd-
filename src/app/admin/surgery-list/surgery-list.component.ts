...
import { BillStatus, Status,PaymentStatus,PaymentMode, DeliveryStatus } from '../../utils/enum';
import {
  ActionModel,
  RequestModel,
  StaffLoginModel,
...
export class SurgeryListComponent {
 DueBill: any = {};
   ...
   selectedBill: any = {};
   SurgerySellListALL: any = {};
   SurgerySellListPayments: any = {};
 
   openViewModal(item: any) {
     this.selectedBill = item;
     $('#viewDetailsModal').modal('show');
     this.SurgerySellList(item);
     this.SurgerySellListPayment(item);
   }
   ...
   SurgerySellList(obj: any) {
     ...
     this.dataLoading = true;
     this.service.SurgerySellList(request).subscribe(
       (r1) => {
         let response = r1 as any;
         if (response.Message == ConstantData.SuccessMessage) {
           this.SurgerySellListALL = response.SurgerySellList;
   ...
   SurgerySellListPayment(obj: any) {
     ...
     this.dataLoading = true;
     this.service.SurgerySellListPayment(request).subscribe(
       (r1) => {
         let response = r1 as any;
         if (response.Message == ConstantData.SuccessMessage) {
           this.SurgerySellListPayments = response.SurgerySellListPayment;
           this.dataLoading = false;
         } else {
           this.toastr.error('Error occured while Fetching  the recored');
         }
       ...
       (response: any) => {
         try {
           ...
           this.router.navigate(['/admin/surgery-bill'], {
             queryParams: {
               id: this.alldata.GetSurgery.SurgeryId,
               redUrl: '/admin/surgery-List',
             },
           });
         } catch (error) {
